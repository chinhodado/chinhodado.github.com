    ...
    BattleDebugger.prototype.displayEventLogAtIndex = function (majorIndex) {
        ...
        for (var p = 1; p <= 2; p++) {
            ...
            for (var f = 0; f < 5; f++) {
                ...
                var finalAtk = this.getFinalStat(originalStats.atk, status.atk, status.isNewLogic[ENUM.StatusType.ATK], status.remainHpAtkUp, hpRatio);
                var finalDef = this.getFinalStat(originalStats.def, status.def, status.isNewLogic[ENUM.StatusType.DEF], status.remainHpDefUp, hpRatio);
                var finalWis = this.getFinalStat(originalStats.wis, status.wis, status.isNewLogic[ENUM.StatusType.WIS], status.remainHpWisUp, hpRatio);
                var finalAgi = this.getFinalStat(originalStats.agi, status.agi, status.isNewLogic[ENUM.StatusType.AGI], status.remainHpAgiUp, hpRatio);
